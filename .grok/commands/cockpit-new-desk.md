...
6. **House, then register, then drivers — same Grok terminal.** Dump full house → **GO** (CONFIRMED + **`commit_on_go` / `go-commit.mjs`**), **SAVE DRAFT** (Grok only), or **EDIT**. **Do not propose FORMING.** When house is **CONFIRMED**, **do not end the session.** Align `08` to that house (add/drop only if they say so) → dump **full `08`** → they say **GO** / **SAVE DRAFT** / **EDIT** → on GO: propose chips → **`commit_on_go` kind=register** → `register-closeout.mjs` **PASS**. **SAVE DRAFT on the register does not start Drivers.** Then print the heading **DRIVERS**. Ask them to name the side of the business they want followed, in their words. You may suggest 2–5 (one house quote and one watching line each — not a second house). They KEEP, SKIP, name their own, or keep none. Empty is valid. Never auto-keep. If their name is already in the house, pin it and do **not** edit the house. If it is not, ask once: house **GO** first, or drop it. Do not rewrite the house from a driver. **GO** → **`commit_on_go` kind=drivers** → `drivers-closeout.mjs` **PASS**. Drivers GO refuses until the register closeout would pass.  
