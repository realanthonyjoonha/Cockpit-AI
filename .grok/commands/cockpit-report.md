---
description: Thesis-lane IB report (deep-dive / earnings-update / initiation) — house+risks first, two checkpoints, PDF, propose_* closeout
...
Parse `$ARGUMENTS`: desk (slug/ticker) · mode (`deep-dive` | `earnings-update` | `initiation`) · register scope (`all` | `pick` | `skim`, default **all**) · optional `R1,R9` when pick · pace (`stop` | `through`, default **stop**) · drivers (`drivers-all` | `drivers-off` | `drivers-pick` + ids; absent means **off**) · optional page budget · optional focus. If desk or mode missing, **ask once** — do not search yet. If glass already set register scope, drivers scope, or pace, do not re-ask. House is always on.
...
4. Report: **delta vs house** always. **register UPDATED** on all/pick only — not a new risks essay. Skim = no register section. **drivers** only when the seed says `drivers-all` or `drivers-pick`: cite, watching, log, still open. Not a second house. No status. `drivers-off` or silence = no engines chapter. A log line is `propose_driver_log` then GO. If it contradicts the house, ask.  
