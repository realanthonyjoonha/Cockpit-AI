...
const attachComplete = attachWorker('TEST', runId, { pid: 1 });
if (attachComplete.ok) bad('attachWorker on complete must be refused');
else ok('attachWorker refuses complete run');
const stillComplete = getResearchRun('TEST', runId);
if (stillComplete.status !== 'complete') bad(`complete mutated to ${stillComplete.status}`);
else ok('complete run stays complete after attach refuse');
...
const inflightBefore = findInFlightRun('THORPH', { job: 'thesis_report' });
if (!inflightBefore || inflightBefore.run_id !== thOrph.run_id) bad('thesis still in-flight before cancel');
else ok('queued thesis is in-flight before cancel');
cancelResearchRun('THORPH', thOrph.run_id);
const inflightAfter = findInFlightRun('THORPH', { job: 'thesis_report' });
if (inflightAfter) bad(`cancelled thesis still in-flight ${inflightAfter.run_id}`);
else ok('cancel drops thesis from in-flight (START COMPILING clears)');
