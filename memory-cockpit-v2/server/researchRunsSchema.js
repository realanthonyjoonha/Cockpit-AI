...
import { filingMapPublishViolations } from './filingMapSchema.js';

export const RESEARCH_RUNS_SCHEMA_VERSION = 1;

export const RESEARCH_JOBS = new Set([
  'deep_compile', 'print_package', 'pack_refresh', 'thesis_report', 'model_read', 'filing_map',
]);
...
export function isFilingMapJob(job) {
  return String(job || '') === 'filing_map';
}

/** Interactive OPEN GROK jobs — no headless pid; orphan sweeper must skip. */
export function isInteractiveResearchJob(job) {
  return isThesisReportJob(job) || isModelReadJob(job) || isFilingMapJob(job);
}
...
export function jobMatchesLane(job, lane) {
  ...
  if (l === 'filings' || l === 'filing_map' || l === 'filing-map') return isFilingMapJob(job);
  if (l === 'compile' || l === 'research') {
    return !isThesisReportJob(job) && !isModelReadJob(job) && !isFilingMapJob(job);
  }
  return true;
}
...
export function validateResearchRunPublish(raw, opts = {}) {
  ...
  if (opts.requireComplete !== false && status === 'complete') {
    ...
    } else if (isFilingMapJob(job)) {
      if (!summary) errors.push('complete filing_map needs a summary');
      if (raw.promotion && raw.promotion.pack_claims) {
        errors.push('filing_map is ops — never pack SoR');
      }
      const rows = Array.isArray(raw.rows) ? raw.rows
        : (Array.isArray(raw.delta?.rows) ? raw.delta.rows : []);
      errors.push(...filingMapPublishViolations({
        runDir: opts.runDir || null,
        rows,
    ...
    const grounded = isFilingMapJob(job)
      ? { errors: [], warnings: [], extracts: { financials: [], risks: [], narrative: [], guide: [] } }
      : applyPublishTruthGate({
    ...
    return {
      ...
      snapshot: {
        ...
        promotion: raw.promotion && typeof raw.promotion === 'object' ? {
          ...
          pack_claims: (isThesisReportJob(job) || isModelReadJob(job) || isFilingMapJob(job)) ? false : !!raw.promotion.pack_claims,
          ...
          model_pack_layers: (isThesisReportJob(job) || isModelReadJob(job) || isFilingMapJob(job)) ? false : !!raw.promotion.model_pack_layers,
...
export function indexRowFromMeta(meta) {
  ...
  return {
    ...
    n_gaps,
    n_claims,
    summary: str(meta.summary).replace(/\s+/g, ' ').slice(0, 240) || null,
...
export function humanJobLabel(job) {
  ...
  if (j === 'filing_map') return 'Filing map';
  return 'Deep compile';
}
