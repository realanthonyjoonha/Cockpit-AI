...
export function mountThinDesks(app, { j, ja }) {
  ...
  app.get('/api/:slug/learn', j(withDesk((rt) => rt.model.learn())));
  app.post('/api/:slug/learn/primer', j(withDesk((rt, req) => rt.model.learnPrimer(req.body || {}))));
  app.post('/api/:slug/learn/map', j(withDesk((rt, req) => rt.model.learnMapPublish(req.body || {}))));
  app.post('/api/:slug/learn/learner', j(withDesk((rt, req) => rt.model.learnLearner(req.body || {}))));
  app.post('/api/:slug/learn/lessons', j(withDesk((rt, req) => rt.model.learnLessonPublish(req.body || {}))));
  app.post('/api/:slug/learn/photos', ja(withDesk(async (rt, req) => rt.model.learnPhotoPublish(req.body || {}))));
  app.get('/api/:slug/learn/photos/:file', (req, res) => {
    try {
      const rt = resolveThinDesk(req.params.slug);
      if (!rt) deskNotFound(req.params.slug);
      const out = rt.model.learnPhotoFile(req.params.file);
  ...
  app.get('/api/:slug/learn/lessons/:id', (req, res) => {
    try {
      const rt = resolveThinDesk(req.params.slug);
      if (!rt) deskNotFound(req.params.slug);
      const out = rt.model.learnLessonGet(req.params.id);
      if (!out?.ok) return res.status(404).json({ ok: false, error: out?.error || 'not found' });
      return res.json(out);
    } catch (e) {
      const status = e.status || 500;
  ...
  app.post('/api/:slug/learn/deep', j(withDesk((rt, req) => rt.model.learnDeepPublish(req.body || {}))));
  app.post('/api/:slug/learn/deep/:id/start', j(withDesk((rt, req) => rt.model.learnDeepStart(req.params.id))));
  app.get('/api/:slug/learn/deep/:id', (req, res) => {
    try {
      const rt = resolveThinDesk(req.params.slug);
      if (!rt) deskNotFound(req.params.slug);
      const out = rt.model.learnDeepGet(req.params.id);
      if (!out?.ok) return res.status(404).json({ ok: false, error: out?.error || 'not found' });
      return res.json(out);
    } catch (e) {
      const status = e.status || 500;
  ...
  app.post('/api/:slug/research/runs/:runId/propose-from-map', j(withDesk((rt, req) => {
    const q = req.query || {};
    const body = req.body || {};
    const dry = body.dry_run === true || body.dryRun === true
      || q.dry_run === '1' || q.dry_run === 'true' || q.dryRun === '1';
    return rt.model.researchProposeFromMap(req.params.runId, { ...body, dry_run: dry });
  })));
  app.post('/api/:slug/research/runs/:runId/propose-from-run', j(withDesk((rt, req) => {
    const q = req.query || {};
    const body = req.body || {};
    const dry = body.dry_run === true || body.dryRun === true
      || q.dry_run === '1' || q.dry_run === 'true' || q.dryRun === '1';
    return rt.model.researchProposeFromRun(req.params.runId, { ...body, dry_run: dry });
  })));
